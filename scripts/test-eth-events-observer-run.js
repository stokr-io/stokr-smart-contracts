...
const loadAddresses = async () => {
    ...
    tokens = [];
    for (let i = 0; i < addresses.contracts.tokens.length; ++i) {
        let address = addresses.contracts.tokens[i];
        let token = await Token.at(address);
        tokens.push(token);
        console.log(`- token ${await token.symbol()} at ${token.address}`);
    }
    sales = [];
    for (let i = 0; i < addresses.contracts.sales.length; ++i) {
        let address = addresses.contracts.sales[i];
