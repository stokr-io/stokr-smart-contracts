...
contract(
  ...
  ([
    ...
    const deployWhitelistAndToken = async () => {
      ...
      let token = await StokrToken.new(
        tokenName,
        tokenSymbol,
        whitelist.address,
        profitDepositor,
    ...
    context("deployment", () => {
      ...
      describe("with invalid parameters", () => {
        it("fails if whitelist is zero address", async () => {
          let reason = await reject.deploy(
            StokrToken.new(
              tokenName,
              tokenSymbol,
              ZERO_ADDRESS,
      ...
      describe("with valid parameters", () => {
        ...
        it("succeeds", async () => {
          ...
          expect(await web3.eth.getCode(token.address)).to.be.not.oneOf([
            "0x",
            "0x0",
          ]);
        });
    ...
    context("address change", () => {
      ...
      describe("of whitelist", () => {
        ...
        it("gets logged", async () => {
          ...
          let entry = tx.logs.find(
            (entry) => entry.event === "WhitelistChange"
          );
      ...
      describe("of profit depositor", () => {
        ...
        it("gets logged", async () => {
          ...
          let newProfitDepositor = randomAddress();
          let tx = await token.setProfitDepositor(newProfitDepositor, {
            from: owner,
          });
          let entry = tx.logs.find(
            (entry) => entry.event === "ProfitDepositorChange"
          );
          expect(entry).to.exist;
        ...
        it("doesn't get logged if set to same address again", async () => {
          ...
          let tx = await token.setProfitDepositor(profitDepositor, {
            from: owner,
          });
          let entry = tx.logs.find(
            (entry) => entry.event === "ProfitDepositorChange"
      ...
      describe("of profit distributor", () => {
        ...
        it("is possible", async () => {
          ...
          await token.setProfitDistributor(newProfitDistributor, {
            from: owner,
          });
          expect(await token.profitDistributor()).to.equal(
            newProfitDistributor
          );
        });
        ...
        it("gets logged", async () => {
          ...
          let newProfitDistributor = randomAddress();
          let tx = await token.setProfitDistributor(newProfitDistributor, {
            from: owner,
          });
          let entry = tx.logs.find(
            (entry) => entry.event === "ProfitDistributorChange"
          );
          expect(entry).to.exist;
        ...
        it("doesn't get logged if set to same address again", async () => {
          ...
          let tx = await token.setProfitDistributor(profitDistributor, {
            from: owner,
          });
          let entry = tx.logs.find(
            (entry) => entry.event === "ProfitDistributorChange"
      ...
      describe("of token recoverer", () => {
        ...
        it("gets logged", async () => {
          ...
          let newTokenRecoverer = randomAddress();
          let tx = await token.setTokenRecoverer(newTokenRecoverer, {
            from: owner,
          });
          let entry = tx.logs.find(
            (entry) => entry.event === "TokenRecovererChange"
          );
          expect(entry).to.exist;
        ...
        it("doesn't get logged if set to same address again", async () => {
          ...
          let tx = await token.setTokenRecoverer(tokenRecoverer, {
            from: owner,
          });
          let entry = tx.logs.find(
            (entry) => entry.event === "TokenRecovererChange"
    ...
    describe("minting", () => {
      ...
      context("until finish", () => {
        ...
        it("gets logged", async () => {
          ...
          let transferEntry = tx.logs.find(
            (entry) => entry.event === "Transfer"
          );
    ...
    describe("transfer", () => {
      ...
      afterEach(
        "invariant: sum of all token balances equals total supply",
        async () => {
          let totalSupply = await token.totalSupply();
      ...
      context("while minting", () => {
        it("is forbidden", async () => {
          expect(await token.canTransfer(debited, credited, toBN(1))).to.be
            .false;
          let reason = await reject.call(
      ...
      context("after minting finished", () => {
        ...
        it("is forbidden if amount exceed balance", async () => {
          ...
          expect(await token.canTransfer(debited, credited, amount)).to.be
            .false;
          let reason = await reject.call(
        ...
        it("to zero address is forbidden", async () => {
          ...
          expect(await token.canTransfer(debited, ZERO_ADDRESS, toBN(0))).to.be
            .false;
          let reason = await reject.call(
        ...
        it("decreases debited balance", async () => {
          ...
          expect(await token.canTransfer(debited, credited, amount)).to.be.true;
          await token.transfer(credited, amount, { from: debited });
          expect(await token.balanceOf(debited)).to.be.bignumber.equal(
            balance.sub(amount)
          );
        });
        ...
        it("increases credited balance", async () => {
          ...
          expect(await token.canTransfer(debited, credited, amount)).to.be.true;
          await token.transfer(credited, amount, { from: debited });
          expect(await token.balanceOf(credited)).to.be.bignumber.equal(
            balance.add(amount)
          );
        });
      });
    ...
    describe("approval", () => {
      ...
      context("after minting finished", () => {
        ...
        it("sets correct allowance", async () => {
          ...
          await token.approve(trustee, amount, { from: approver });
          expect(
            await token.allowance(approver, trustee)
        ...
        it("increasing gets logged", async () => {
          let amount = (await token.balanceOf(approver)).add(toBN(1));
          await token.approve(trustee, amount, { from: approver });
          let tx = await token.increaseAllowance(trustee, toBN(1), {
            from: approver,
          });
          let entry = tx.logs.find((entry) => entry.event === "Approval");
          expect(entry).to.exist;
        ...
        it("decreasing gets logged", async () => {
          let amount = (await token.balanceOf(approver)).add(toBN(1));
          await token.approve(trustee, amount, { from: approver });
          let tx = await token.decreaseAllowance(trustee, toBN(1), {
            from: approver,
          });
          let entry = tx.logs.find((entry) => entry.event === "Approval");
          expect(entry).to.exist;
    ...
    describe("approved transfer", () => {
      ...
      context("while minting", () => {
        it("is forbidden", async () => {
          expect(
            await token.canTransferFrom(trustee, debited, credited, toBN(0))
          ).to.be.false;
          let reason = await reject.call(
      ...
      context("after minting finished", () => {
        ...
        it("is forbidden if debited account isn't whitelisted", async () => {
          ...
          await whitelist.removeFromWhitelist([debited], { from: owner });
          expect(
            await token.canTransferFrom(trustee, debited, credited, amount)
          ).to.be.false;
          let reason = await reject.call(
        ...
        it("is forbidden if credited account is zero", async () => {
          ...
          await whitelist.addToWhitelist([ZERO_ADDRESS], { from: owner });
          expect(
            await token.canTransferFrom(trustee, debited, ZERO_ADDRESS, amount)
          ).to.be.false;
          let reason = await reject.call(
        ...
        it("is forbidden if amount exceeds allowance", async () => {
          ...
          let reason = await reject.call(
            token.transferFrom(debited, credited, amount.add(toBN(1)), {
              from: trustee,
            })
          );
          expect(reason).to.equal("Amount exceeds allowance");
        ...
        it("is possible", async () => {
          ...
          let tx = await token.transferFrom(debited, credited, amount, {
            from: trustee,
          });
          let entry = tx.logs.find((entry) => entry.event === "Transfer");
          expect(entry).to.exist;
        ...
        it("decreases allowance", async () => {
          ...
          expect(await token.allowance(debited, trustee)).to.be.bignumber.equal(
        ...
        it("decreases debited balance", async () => {
          ...
          await token.transferFrom(debited, credited, amount, {
            from: trustee,
          });
          expect(await token.balanceOf(debited)).to.be.bignumber.equal(
            balance.sub(amount)
    ...
    describe("profit deposit", () => {
      ...
      context("after minting finished", () => {
        ...
        it("via fallback function is possible", async () => {
          ...
          expect(
            toBN(await web3.eth.getBalance(token.address))
        ...
        it("increases totalProfits", async () => {
          ...
          let value = ether(2);
          await token.depositProfit({ from: profitDepositor, value });
          expect(await token.totalProfits()).to.be.bignumber.equal(
            profits.add(value)
          );
        });
    ...
    describe("profit share", () => {
      ...
      const getAccount = async (address) => {
        let { balance, lastTotalProfits, profitShare } = await token.accounts(
          address
        );
      ...
      afterEach(
        "invariant: sum of individual profits equals token wei balance",
        async () => {
          let asset = toBN(await web3.eth.getBalance(token.address));
          let shares = toBN(0);
      ...
      context("while minting", () => {
        ...
        it("withdrawal is forbidden", async () => {
          for (let investor of investors.values()) {
            let reason = await reject.call(
              token.withdrawProfitShare({ from: investor })
            );
            expect(reason).to.equal("Total supply may change");
          }
        });
      ...
      context("after minting finished", () => {
        ...
        it("is correctly calculated as share of investors' token balances", async () => {
          ...
          for (let investor of investors.values()) {
            ...
            expect(owing.sub(share).mul(supply)).to.be.bignumber.equal(
              balance.mul(profits)
            );
          }
        });
        ...
        it("is correctly calculated after some more profit was deposited", async () => {
          ...
          for (let investor of investors.values()) {
            ...
            expect(owing.mul(supply)).to.be.bignumber.equal(
              balance.mul(profits)
            );
          }
        });
        ...
        it("is correctly calculated after some tokens transfer and profit deposit", async () => {
          ...
          expect(owing1.sub(share1).mul(supply)).to.be.bignumber.equal(
            balance1.mul(value)
          );
          expect(owing2.sub(share2).mul(supply)).to.be.bignumber.equal(
            balance2.mul(value)
          );
        });
        ...
        it("can be withdrawn", async () => {
          ...
          let entry = tx.logs.find(
            (entry) => entry.event === "ProfitShareWithdrawal"
          );
          expect(entry).to.exist;
          expect(entry.args.investor).to.equal(investor1);
          expect(entry.args.beneficiary).to.equal(investor1);
          expect(entry.args.amount).to.be.bignumber.equal(owing);
          expect((await getAccount(investor1)).profitShare).to.be.bignumber
            .zero;
          expect(
            toBN(await web3.eth.getBalance(token.address))
        ...
        it("can be withdrawn to third party", async () => {
          ...
          let tx = await token.withdrawProfitShareTo(beneficiary, {
            from: investor2,
          });
          let entry = tx.logs.find(
            (entry) => entry.event === "ProfitShareWithdrawal"
          ...
          expect(entry.args.amount).to.be.bignumber.equal(owing);
          expect((await getAccount(investor2)).profitShare).to.be.bignumber
            .zero;
          expect(
        ...
        it("withdrawal to many by anyone is forbidden", async () => {
          let reason = await reject.call(
            token.withdrawProfitShares(investors, { from: anyone })
          );
        ...
        it("can be withdrawn for many", async () => {
          ...
          let tx = await token.withdrawProfitShares(investors, {
            from: profitDistributor,
          });
          let entries = tx.logs.filter(
            (entry) => entry.event === "ProfitShareWithdrawal"
          );
          expect(entries).to.have.lengthOf(investors.length);
          let entry = entries.find(
            (entry) => entry.args.investor === investor3
          );
          expect(entry).to.exist;
          expect(entry.args.beneficiary).to.equal(investor3);
          expect((await getAccount(investor3)).profitShare).to.be.bignumber
            .zero;
          expect(
            toBN(await web3.eth.getBalance(investor3))
          ).to.be.bignumber.equal(asset.add(owing));
        });
      });
    });
    ...
    describe("token recovery", () => {
      ...
      it("is possible", async () => {
        ...
        await token.recoverToken(investor2, investor3, {
          from: tokenRecoverer,
        });
        ...
        expect(newAccount.lastTotalProfits).to.be.bignumber.equal(
          account.lastTotalProfits
        );
        expect(newAccount.profitShare).to.be.bignumber.equal(
          account.profitShare
        );
      });

      it("gets logged", async () => {
        let tx = await token.recoverToken(investor3, investor2, {
          from: tokenRecoverer,
        });
    ...
    describe("destruction", () => {
      ...
      it("transfers ether balance to owner and gets logged", async () => {
        ...
        expect(entry).to.exist;
        expect(toBN(await web3.eth.getBalance(owner))).to.be.bignumber.equal(
          balance.add(value)
        );
      });
    });
