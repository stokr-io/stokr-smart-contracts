...
contract(
  "StokrCrowdsale",
  ([owner, companyWallet, reserveAccount, investor1, investor2, anyone]) => {
    ...
    const deployToken = async (params) => {
      ...
      await whitelist.addToWhitelist(
        [companyWallet, reserveAccount, investor1, investor2],
        { from: owner }
      );

      return StokrToken.new(
        "Sample Stokr Token",
        "STOKR",
        whitelist.address,
        randomAddress(),
    ...
    context("deployment", () => {
      describe("with invalid parameters", () => {
        ...
        it("fails if token purchase minimum exceeds public sale cap", async () => {
          let params = await getDeployParams();
          params.tokenPurchaseMinimum = params.tokenCapOfPublicSale.add(
            toBN(1)
          );
          let reason = await reject.deploy(deploySale(params));
        ...
        it("fails if token purchase minimum exceeds private sale cap", async () => {
          let params = await getDeployParams();
          params.tokenPurchaseMinimum = params.tokenCapOfPrivateSale.add(
            toBN(1)
          );
          let reason = await reject.deploy(deploySale(params));
          expect(reason).to.equal("Purchase minimum exceeds cap");
      ...
      describe("with valid parameters", () => {
        ...
        it("succeeds", async () => {
          ...
          expect(await web3.eth.getCode(sale.address)).to.be.not.oneOf([
            "0x",
            "0x0",
          ]);
        });
        ...
        it("sets correct token cap of public sale", async () => {
          expect(await sale.tokenCapOfPublicSale()).to.be.bignumber.equal(
            params.tokenCapOfPublicSale
          );
        });

        it("sets correct token cap of private sale", async () => {
          expect(await sale.tokenCapOfPrivateSale()).to.be.bignumber.equal(
            params.tokenCapOfPrivateSale
          );
        });

        it("sets correct token goal", async () => {
          expect(await sale.tokenGoal()).to.be.bignumber.equal(
            params.tokenGoal
          );
        });

        it("sets correct token purchase minimum", async () => {
          expect(await sale.tokenPurchaseMinimum()).to.be.bignumber.equal(
            params.tokenPurchaseMinimum
          );
        });

        it("sets correct token purchase limit", async () => {
          expect(await sale.tokenPurchaseLimit()).to.be.bignumber.equal(
            params.tokenPurchaseLimit
          );
        });

        it("sets correct token reserve per mill", async () => {
          expect(await sale.tokenReservePerMill()).to.be.bignumber.equal(
            params.tokenReservePerMill
          );
        });

        it("sets correct token price", async () => {
          expect(await sale.tokenPrice()).to.be.bignumber.equal(
            params.tokenPrice
          );
        });

        it("sets correct opening time", async () => {
          expect(await sale.openingTime()).to.be.bignumber.equal(
            params.openingTime
          );
        });

        it("sets correct closing time", async () => {
          expect(await sale.closingTime()).to.be.bignumber.equal(
            params.closingTime
          );
        });

        it("sets correct limit end time", async () => {
          expect(await sale.limitEndTime()).to.be.bignumber.equal(
            params.limitEndTime
          );
        });
        ...
        it("correctly calculates remaining sale time", async () => {
          ...
          expect(await sale.timeRemaining()).to.be.bignumber.least(
            toBN(params.closingTime - params.openingTime)
          );
        });
    ...
    context("before sale opening", () => {
      ...
      describe("change of opening time", async () => {
        it("by anyone but owner is forbidden", async () => {
          let openingTime = (await sale.openingTime()).add(toBN(time.mins(1)));
          let reason = await reject.call(
            sale.changeOpeningTime(openingTime, { from: anyone })
          );
          expect(reason).to.equal("Restricted to owner");
        });

        it("to past is forbidden", async () => {
          let openingTime = time.now();
          let reason = await reject.call(
            sale.changeOpeningTime(openingTime, { from: owner })
          );
          expect(reason).to.equal("OpeningTime not in the future");
        });

        it("is possible", async () => {
          let openingTime = (await sale.openingTime()).add(toBN(time.days(14)));
          await sale.changeOpeningTime(openingTime, { from: owner });
          expect(await sale.openingTime()).to.be.bignumber.equal(openingTime);
        });

        it("doesn't affect offering duration", async () => {
          let timeShift = toBN(time.days(14));
          let openingTime = (await sale.openingTime()).add(timeShift);
          let closingTime = (await sale.closingTime()).add(timeShift);
          await sale.changeOpeningTime(openingTime, { from: owner });
        ...
        it("gets logged", async () => {
          let timeShift = toBN(time.days(14));
          let oldOpeningTime = await sale.openingTime();
          let oldClosingTime = await sale.closingTime();
          let newOpeningTime = oldOpeningTime.add(timeShift);
          let newClosingTime = oldClosingTime.add(timeShift);
          let tx = await sale.changeOpeningTime(newOpeningTime, {
            from: owner,
          });
          let entry;
          entry = tx.logs.find((entry) => entry.event === "OpeningTimeChange");
          expect(entry).to.exist;
          expect(entry.args.previous).to.be.bignumber.equal(oldOpeningTime);
          expect(entry.args.current).to.be.bignumber.equal(newOpeningTime);
          entry = tx.logs.find((entry) => entry.event === "ClosingTimeChange");
          expect(entry).to.exist;
      ...
      describe("change of closing time", async () => {
        it("by anyone but owner is forbidden", async () => {
          ...
          let reason = await reject.call(
            sale.changeClosingTime(closingTime, { from: anyone })
          );
          expect(reason).to.equal("Restricted to owner");
        });
        ...
        it("to past is forbidden", async () => {
          ...
          let reason = await reject.call(
            sale.changeClosingTime(closingTime, { from: owner })
          );
        ...
        it("is possible", async () => {
          let closingTime = (await sale.openingTime()).add(
            toBN(MAX_OFFERING_PERIOD)
          );
        ...
        it("gets logged", async () => {
          let oldClosingTime = await sale.closingTime();
          let newClosingTime = (await sale.openingTime()).add(
            toBN(time.secs(1))
          );
          let tx = await sale.changeClosingTime(newClosingTime, {
            from: owner,
          });
          let entry = tx.logs.find(
            (entry) => entry.event === "ClosingTimeChange"
          );
          expect(entry).to.exist;
          expect(entry.args.previous).to.be.bignumber.equal(oldClosingTime);
          expect(entry.args.current).to.be.bignumber.equal(newClosingTime);
      ...
      describe("token distribution", () => {
        [
          ...
          it(`via ${fns.name} with #accounts < #amounts is forbidden`, async () => {
            let reason = await reject.call(
              sale[fns.distributeTokens]([investor1, investor2], [1, 2, 3], {
                from: owner,
              })
            );
          ...
          it(`via ${fns.name} with #accounts > #amounts is forbidden`, async () => {
            let reason = await reject.call(
              sale[fns.distributeTokens]([investor1, investor2], [1], {
                from: owner,
              })
            );
            expect(reason).to.equal("Lengths are different");
          ...
          it(`via ${fns.name} is possible`, async () => {
            ...
            await sale[fns.distributeTokens](
              [investor1, investor2],
              [amount1, amount2],
              { from: owner }
            );
          ...
          it(`via ${fns.name} gets logged`, async () => {
            ...
            let tx = await sale[fns.distributeTokens]([investor1], [amount], {
              from: owner,
            });
            let entry = tx.logs.find(
              (entry) => entry.event === "TokenDistribution"
            );
            expect(entry).to.exist;
          ...
          it(`via ${fns.name} increases recipients' balance`, async () => {
            ...
            expect(await token.balanceOf(investor1)).to.be.bignumber.equal(
              balance1.add(amount1)
            );
            expect(await token.balanceOf(investor2)).to.be.bignumber.equal(
              balance2.add(amount2)
            );
          });
          ...
          it(`via ${fns.name} increases token total supply`, async () => {
            ...
            expect(await token.totalSupply()).to.be.bignumber.equal(
              supply.add(amount1).add(amount2)
            );
          });
          ...
          it(`via ${fns.name} decreases remaining tokens`, async () => {
            ...
            expect(await sale[fns.tokenRemaining]()).to.be.bignumber.equal(
              remaining.sub(amount1).sub(amount2)
            );
          });
          ...
          it(`via ${fns.name} increases sold tokens`, async () => {
            ...
            expect(await sale.tokenSold()).to.be.bignumber.equal(
              sold.add(toBN(amount1)).add(toBN(amount2))
            );
          });

          it(`via ${fns.name} may reach goal`, async () => {
            let amount = (await sale.tokenGoal()).sub(await sale.tokenSold());
            await sale[fns.distributeTokens]([investor1], [amount], {
              from: owner,
            });
      ...
      describe("token purchase", () => {
        it("is forbidden", async () => {
          let reason = await reject.call(
            sale.buyTokens({ from: investor1, value: ether(1) })
          );
      ...
      describe("investor refund", () => {
        ...
        it("distribution is forbidden", async () => {
          let reason = await reject.call(
            sale.distributeRefunds([investor1], { from: owner })
          );
          expect(reason).to.equal("Sale has not been finalized");
        });
      });
    ...
    context("while sale is open", () => {
      ...
      describe("change of opening time", async () => {
        it("is forbidden", async () => {
          ...
          expect(reason).to.equal("Sale has started already");
        });
      });
      ...
      describe("token distribution", () => {
        it("via public sale is possible", async () => {
          await sale.distributeTokensViaPublicSale([investor1], [1], {
            from: owner,
          });
        });

        it("via private sale is possible", async () => {
          await sale.distributeTokensViaPrivateSale([investor1], [1], {
            from: owner,
          });
        });
        ...
        it("via public sale forwards funds if goal was reached", async () => {
          ...
          expect(toBN(await web3.eth.getBalance(sale.address))).to.be.not
            .bignumber.zero;
          await sale.distributeTokensViaPublicSale([investor1], [goal]);
          expect(toBN(await web3.eth.getBalance(sale.address))).to.be.bignumber
            .zero;
        });
      ...
      describe("token purchase", () => {
        it("by non-whitelisted is forbidden", async () => {
          let reason = await reject.call(
            sale.buyTokens({ from: anyone, value: ether(1) })
          );
        ...
        it("below purchase minimum is forbidden", async () => {
          ...
          let reason = await reject.call(
            sale.buyTokens({ from: investor1, value })
          );
        ...
        it("is possible", async () => {
          ...
          await sale.buyTokens({ from: investor1, value: ether(1) });
          expect(await token.balanceOf(investor1)).to.be.bignumber.above(
            balance
          );
        });
        ...
        it("decreases investor's wei balance", async () => {
          ...
          await sale.buyTokens({ from: investor1, value });
          expect(
            toBN(await web3.eth.getBalance(investor1))
          ).to.be.bignumber.most(asset.sub(value));
        });
        ...
        it("exceeding remaining tokens for public sale is forbidden", async () => {
          let remaining = await sale.tokenRemainingForPublicSale();
          let value = (await tokenValueOf(sale, remaining)).add(
            await valueOf1(sale)
          );
          let reason = await reject.call(
            sale.buyTokens({ from: investor1, value })
        ...
        it("is stored if goal wasn't reached", async () => {
          ...
          await sale.buyTokens({ from: investor1, value });
          expect(await sale.investments(investor1)).to.be.bignumber.equal(
            value
          );
          await sale.buyTokens({ from: investor1, value });
          expect(await sale.investments(investor1)).to.be.bignumber.equal(
        ...
        it("is not forwarded if goal wasn't reached", async () => {
          ...
          await sale.buyTokens({ from: investor1, value });
          expect(
            toBN(await web3.eth.getBalance(companyWallet))
          ).to.be.bignumber.equal(asset);
        });

        it("may reach goal eventually", async () => {
          let amount = (await sale.tokenGoal()).sub(await sale.tokenSold());
          let value = (await tokenValueOf(sale, amount)).add(
            await valueOf1(sale)
          );
          await sale.buyTokens({ from: investor1, value });
          expect(await sale.goalReached()).to.be.true;
    ...
    context("while sale is open (edge case: purchases are limited)", () => {
      ...
      describe("token purchase", () => {
        ...
        it("beyond limit is forbidden", async () => {
          let maxValue = await tokenValueOf(
            sale,
            await sale.tokenPurchaseLimit()
          );
        ...
        it("increases investor's purchased amount", async () => {
          ...
          let amount = await tokenAmountOf(sale, value);
          await sale.buyTokens({ from: investor1, value });
          expect(await sale.tokenPurchased(investor1)).to.be.bignumber.equal(
            purchased.add(amount)
          );
        });
      });
    });
    ...
    context("after sale (goal missed)", () => {
      ...
      before("deploy", async () => {
        ...
        await token.setMinter(sale.address, { from: owner });
        await time.increaseTo(await sale.limitEndTime());
        let value = (await tokenValueOf(sale, await sale.tokenGoal())).div(
          toBN(2)
        );
        await sale.buyTokens({ from: investor1, value });
        await sale.buyTokens({
          from: investor2,
          value: value.sub(await valueOf1(sale)),
        });
      ...
      describe("sale state", () => {
        ...
        it("sold tokens is below goal", async () => {
          expect(await sale.tokenSold()).to.be.bignumber.below(
            await sale.tokenGoal()
          );
        });
    ...
    context("after sale (goal reached)", () => {
      ...
      before("deploy", async () => {
        ...
        await sale.buyTokens({
          from: investor2,
          value: value.add(await valueOf1(sale)),
        });
        await time.increaseTo(await sale.closingTime());
        startState = await EVM.snapshot();
      });
      ...
      describe("sale state", () => {
        ...
        it("sold tokens is at least goal", async () => {
          expect(await sale.tokenSold()).to.be.bignumber.least(
            await sale.tokenGoal()
          );
        });
    ...
    context("after sale (reached due to sellout)", () => {
      ...
      before("deploy", async () => {
        ...
        await sale.buyTokens({
          from: investor2,
          value: await tokenValueOf(sale, remaining),
        });
        startState = await EVM.snapshot();
      });
      ...
      describe("token distribution", () => {
        it("via public sale is not possible", async () => {
          let reason = await reject.call(
            sale.distributeTokensViaPublicSale([investor1], [1], {
              from: owner,
            })
          );
          expect(reason).to.equal("Not enough tokens available");
    ...
    context("after finalization (goal missed)", () => {
      ...
      describe("token contract", () => {
        it("has been destroyed", async () => {
          expect(await web3.eth.getCode(token.address)).to.be.oneOf([
            "0x",
            "0x0",
          ]);
        });
      ...
      describe("token distribution", () => {
        ...
        it("via private sale is forbidden", async () => {
          let reason = await reject.call(
            sale.distributeTokensViaPrivateSale([investor1], [1], {
              from: owner,
            })
          );
          expect(reason).to.equal("Sale has been finalized");
      ...
      describe("investor refund", () => {
        ...
        it("distribution is possible", async () => {
          await sale.distributeRefunds([investor1, investor2, anyone], {
            from: anyone,
          });
        });
      ...
      [
        ...
        it(`of token distribution via ${fns.name} to many investors`, async () => {
          ...
          while (isFinite(count)) {
            ...
            try {
              let tx = await sale[fns.distributeTokens](investors, amounts, {
                from: owner,
              });
              maximum = count;
