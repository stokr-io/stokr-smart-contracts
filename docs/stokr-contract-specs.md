...
may [transfer the ownership](#ownership-func-transfer) to another address.
...
~ * [transfer ownership](#ownable-func-transfer) to another account
  ...
  * [finalize](#crowdsale-func-finalization) the sale, but only
    ...
    #. after the [public sale](#crowdsale-func-sale) has
       [closed](#crowdsale-timing)
  * [distribute refunds](#crowdsale-func-refunding) to
    [investors](#whitelist-role-investor) who
    [paid in Ether](#crowdsale-func-purchase), but only
...
~ * [purchase tokens](#crowdsale-func-purchase) with Ether in public sale,
    but only
    #. while the sale is [open](#crowdsale-timing)
...
### Public vs. Private Sale  {#crowdsale-sale}
...
Additionally, as long as the public crowdsale is [open](#crowdsale-timing),
...
As long as the public crowdsale is [open](#crowdsale-timing),
investors can [purchase](#crowdsale-func-purchase) tokens directly by sending
Ether to the crowdsale instance.
...
the [public sale](#crowdsale-sale) by sending Ether to the crowdsale instance.
...
~ * only by [whitelisted investors](#whitelist-role-investor)
  * only while the public sale is [open](#crowdsale-timing) that is
...
Restrictions
~ * only by the [owner](#crowdsale-role-owner)
...
Restrictions
~ none, but until after the token's total was fixed (the
...
Restrictions
~ only after the token's total was fixed
